  ...
  refetchRates() {
    fetch("/api/rates")
      ...
      .then(result => {
        ...
        result.forEach(x => {
          if (x["key"] === "USDEUR") rates["USDEUR"] = x["value"]
          else if (x["key"] === "USDGBP") rates["USDGBP"] = x["value"]
          else if (x["key"] === "EURUSD") rates["EURUSD"] = x["value"]
          else if (x["key"] === "EURGBP") rates["EURGBP"] = x["value"]
        })
        ...
        <div className="sidebar">
            ...
            {({ data, loading, error }) => {
              return (
                <div className="store-details">
                  ...
                  {loading && (
                    <span className="store-name">Please wait...</span>
                  )}
                  {!loading && !error && (
                    <span className="store-name">{data.shop.name}</span>
                  )}
                  {error && <p>Error {JSON.stringify(error)}</p>}
                  {!loading && !error && (
                    <a
                      href={"http://" + data.shop.myshopifyDomain}
                      target="_blank"
                    >
          ...
          <div className="menu">
            <div
              ...
              className={
                "item waves-effect waves-light " +
                (this.state.tab === "rates" ? "active" : "")
              }
            >
            ...
            <div
              ...
              className={
                "item waves-effect waves-light " +
                (this.state.tab === "products" ? "active" : "")
              }
            >
            ...
            {this.state.tab === "rates" && (
              <div className="card">
                ...
                <button
                  ...
                  disabled={this.state.savingRates}
                  style={{ marginTop: 25 }}
                >
                  Update Rates
                </button>
                {this.state.savingRates && (
                  <span class="green-text" style={{ marginLeft: 20 }}>
